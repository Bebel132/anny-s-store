...
const Home = () => {
  ...
  const handleSearch = (query: string) => {
    productsService.list(`&$top=6&$filter=contains(tolower(Name),tolower('${query}')) or contains(tolower(Categoria),tolower('${query}'))`).then((res) => {
      setProducts(res.value);
  ...
  return (
    ...
    <div className="flex justify-center flex-col gap-3 w-[1250px] m-auto pt-5">
      <input type="text" name="" id="" onChange={(e) => handleSearch(e.target.value)} className="w-full m-auto border-gray-300 border" placeholder="buscar"/>
      <div className="grid grid-cols-3 gap-10">
        {products.map((product) => (
          <div key={product.Id} className="p-4 w-[350px]">
            <div
            ...
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => handleAddToCart(product)} className="bg-emerald text-white p-2 rounded cursor-pointer hover:bg-emerald-500">
